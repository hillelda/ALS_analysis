...
def pre_process_als(files_location, data_files, df_list, full_add_list, ds_name_list):
    for cur_file in data_files:
        ...
        df['sig up als'] = (df['FDR'] < 0.05) & (df['logFC'] >= 0)
        df['sig down als'] = (df['FDR'] < 0.05) & (df['logFC'] < 0)
        df['unsig up als'] = (df['FDR'] >= 0.05) & (df['logFC'] >= 0)
        df['unsig down als'] = (df['FDR'] >= 0.05) & (df['logFC'] < 0)
...
def pre_process_BoNT(files_location_address, cur_file):
    ...
    # Add one-hot columns for significance and sign
    Bo_NT_df['sig up bont'] = (Bo_NT_df['FDR'] < 0.05) & (Bo_NT_df['logFC'] >= 0)
    Bo_NT_df['sig down bont'] = (Bo_NT_df['FDR'] < 0.05) & (Bo_NT_df['logFC'] < 0)
    Bo_NT_df['unsig up bont'] = (Bo_NT_df['FDR'] >= 0.05) & (Bo_NT_df['logFC'] >= 0)
    Bo_NT_df['unsig down bont'] = (Bo_NT_df['FDR'] >= 0.05) & (Bo_NT_df['logFC'] < 0)
